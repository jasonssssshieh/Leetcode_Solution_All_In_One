...
那么前一步的时候猫或者老鼠一定会尽可能的往能够保证他们自己胜利的地方去走。举个例子，现在cat在x位置，老鼠在y位置，当前回合是cat回合，也就是猫来走当前这一步，我们的状态status可以定义为status[x][y][cat]，那么我们就去寻找，对于x位置，下一步能够走到哪些node，假设下一步的node有： a, b。如果猫走到a点，那么下一步的状态就换成了，status[a][y][mouse]，也就是成了老鼠的回合，如果这个状态下，我们能够确认status[a][y][mouse]是老鼠获胜，那么上一次猫的回合，猫绝对不会走这一步，因此，对于status[x][y][cat] 我们可以走的node就只剩下b。好，假设我们选择了b，，那么下一步的状态就换成了status[b][y][mouse]，也就是成了老鼠的回合，如果这个状态下，我们能够确认status[b][y][mouse]是老鼠获胜，那么猫也还是不会走这一条路，那么status[x][y][mouse]就无路可走，也就是说这时候不管猫如何走，最后都是老鼠获胜，所以status[x][y][mouse]就被“染色”成为“老鼠获胜”。同理，如果status[x][y][mouse]只要能有一条路使得猫获胜，那么猫一定会走这条路，此时status[x][y][mouse]就被“染色”成为“猫获胜”。这个逻辑就：在A的回合，除非是没能找到一条A获胜的路，那么状态要么是对方获胜，要么是平局，否则，只要能有一条路保证A获胜，那么这个status就被染色成“A获胜”。
对于老鼠的回合也是同样的逻辑。

最后没有被染色的那些点，就是结果无法确认的点，也就是代表平局。这个地方博主也有点不是完全彻底的理解，但个人认为就是一个动态博弈的过程，如果这个状态下双方都没法占优，那么就是无法确认，也就是平局。
